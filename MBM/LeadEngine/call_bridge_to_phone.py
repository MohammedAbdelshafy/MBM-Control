...
if not (TWILIO_ACCOUNT_SID and TWILIO_PHONE_NUMBER):
    print("ERROR: TWILIO_ACCOUNT_SID / TWILIO_PHONE_NUMBER must be set in AI/.env")
    sys.exit(1)
...
def bridge_call(my_mobile_number, prospect_number, simulate=False):
    ...
    client = get_client()
    require_live_calls(client, prospect=prospect_number)
