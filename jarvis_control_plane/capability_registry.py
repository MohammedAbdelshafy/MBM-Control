...
def build_capability_registry() -> list[CapabilitySpec]:
    ...
    return [
        ...
        # GTM LAYER (Part 4): converge onto observed gtm/factory/control-plane tools.
        # No second registry: same spec, same policy mapping, same audit fields.
        CapabilitySpec(
            capability="market_research", provider="gtm", tool="slice_a.run_slice_a",
            permission="READ_ONLY",
            input_schema=_schema(["signals"], {"signals": "array"}),
            output_schema=_schema([], {"opportunities": "array"}),
            side_effect="none", approval_required=False,
            factory_stages=["DISCOVER", "RESEARCH"],
            status="INTEGRATED", reason="offline signal discovery; agent contracts deferred (stubs)",
        ),
        CapabilitySpec(
            capability="company_research", provider="gtm", tool="webfetch",
            permission="READ_ONLY",
            input_schema=_schema(["url"], {"url": "string"}),
            output_schema=_schema([], {"profile": "object"}),
            side_effect="none", approval_required=False,
            factory_stages=["RESEARCH"],
            status="INTEGRATED", reason="allowlisted fetch + provenance required downstream; ACCOUNT_RESEARCHER stub deferred",
        ),
        CapabilitySpec(
            capability="prospect_discovery", provider="gtm", tool="fixtures_only",
            permission="READ_ONLY",
            input_schema=_schema(["criteria"], {"criteria": "object"}),
            output_schema=_schema([], {"prospects": "array"}),
            side_effect="none", approval_required=False,
            factory_stages=["DISCOVER"],
            status="DEFERRED", reason="no authorized prospect source wired; INTENT_HUNTER/FACEBOOK_INTEL are stubs",
        ),
        CapabilitySpec(
            capability="lead_enrichment", provider="gtm", tool="fixtures_only",
            permission="READ_ONLY",
            input_schema=_schema(["company"], {"company": "string"}),
            output_schema=_schema([], {"profile": "object"}),
            side_effect="none", approval_required=False,
            factory_stages=["RESEARCH"],
            status="DEFERRED", reason="no verified enricher; enrichment without provenance is fabrication",
        ),
        CapabilitySpec(
            capability="lead_cleaning", provider="gtm", tool="run_cleaner",
            permission="SAFE_WRITE",
            input_schema=_schema(["input_path"], {"input_path": "string"}),
            output_schema=_schema([], {"summary": "object"}),
            side_effect="scoped_artifact_write", approval_required=False,
            factory_stages=["SCORE"],
            status="INTEGRATED", reason="p4 cleaner + canonical gate + dedupe; demo-verified",
        ),
        CapabilitySpec(
            capability="lead_qualification", provider="gtm", tool="production_gate.evaluate_gate",
            permission="READ_ONLY",
            input_schema=_schema(["entity_id"], {"entity_id": "string"}),
            output_schema=_schema([], {"can_execute": "string"}),
            side_effect="none", approval_required=False,
            factory_stages=["SCORE"],
            status="INTEGRATED", reason="6-point gate (evidence/contactable/suppression/confidence/channel/approval)",
        ),
        CapabilitySpec(
            capability="account_scoring", provider="gtm", tool="score_account",
            permission="READ_ONLY",
            input_schema=_schema(["signals"], {"signals": "object"}),
            output_schema=_schema([], {"score": "string"}),
            side_effect="none", approval_required=False,
            factory_stages=["SCORE"],
            status="INTEGRATED", reason="transparent weighted factors; advisory only, never auto-qualifies",
        ),
        CapabilitySpec(
            capability="offer_matching", provider="gtm", tool="choose_revenue_route",
            permission="READ_ONLY",
            input_schema=_schema(["opportunity_id"], {"opportunity_id": "string"}),
            output_schema=_schema([], {"rail": "string"}),
            side_effect="none", approval_required=False,
            factory_stages=["STRATEGY"],
            status="INTEGRATED", reason="proposal-only revenue router; AI_FIT_ARCHITECT stub deferred",
        ),
        CapabilitySpec(
            capability="message_generation", provider="gtm", tool="draft_outreach",
            permission="SAFE_WRITE",
            input_schema=_schema(["company"], {"company": "string"}),
            output_schema=_schema([], {"draft": "object"}),
            side_effect="draft_artifact", approval_required=False,
            factory_stages=["STRATEGY", "BUILD"],
            status="INTEGRATED", reason="evidence-interpolated drafts; banned claims fail closed; PERSONALIZER stub deferred",
        ),
        CapabilitySpec(
            capability="outreach_drafting", provider="gtm", tool="crm_overlay_proposal",
            permission="CONTROLLED_WRITE",
            input_schema=_schema(["entity_id"], {"entity_id": "string"}),
            output_schema=_schema([], {"proposal": "object"}),
            side_effect="proposal_artifact", approval_required=True,
            factory_stages=["PACKAGE"],
            status="INTEGRATED", reason="campaign preparation is controlled; sending stays BLOCKED",
        ),
        CapabilitySpec(
            capability="outreach_send", provider="gtm", tool="gmail_dispatcher",
            permission="CONSEQUENTIAL_EXTERNAL_ACTION",
            input_schema=_schema(["entity_id"], {"entity_id": "string"}),
            output_schema=_schema([], {"send_result": "object"}),
            side_effect="external_send", approval_required=True,
            factory_stages=[],
            status="BLOCKED", reason="requires production-gate HUMAN_APPROVED + credentials; never automatic; no creds observed",
        ),
        CapabilitySpec(
            capability="response_classification", provider="gtm", tool="response_classifier",
            permission="READ_ONLY",
            input_schema=_schema(["text"], {"text": "string"}),
            output_schema=_schema([], {"label": "string"}),
            side_effect="none", approval_required=False,
            factory_stages=["MEASURE"],
            status="INTEGRATED", reason="deterministic labels; UNSUBSCRIBE routes to immediate suppression",
        ),
        CapabilitySpec(
            capability="crm_update", provider="gtm", tool="crm_overlay_proposal",
            permission="CONTROLLED_WRITE",
            input_schema=_schema(["entity_id"], {"entity_id": "string"}),
            output_schema=_schema([], {"proposal": "object"}),
            side_effect="proposal_only_no_mutation", approval_required=True,
            factory_stages=["MEASURE", "LEARN"],
            status="INTEGRATED", reason="overlay never mutates production CRM; HubSpot/Airtable MCPs unobserved (deferred)",
        ),
        CapabilitySpec(
            capability="pipeline_management", provider="gtm", tool="GtmStateMachine.transition",
            permission="CONTROLLED_WRITE",
            input_schema=_schema(["target_state"], {"target_state": "string"}),
            output_schema=_schema([], {"state": "string"}),
            side_effect="in_memory_state", approval_required=True,
            factory_stages=["MEASURE"],
            status="INTEGRATED", reason="14-state validated transitions; illegal moves fail closed",
        ),
        CapabilitySpec(
            capability="sales_brief", provider="gtm", tool="build_sales_brief",
            permission="READ_ONLY",
            input_schema=_schema(["company"], {"company": "string"}),
            output_schema=_schema([], {"brief": "object"}),
            side_effect="none", approval_required=False,
            factory_stages=["STRATEGY"],
            status="INTEGRATED", reason="verified-fields-only briefs; no commitments (human_review next)",
        ),
        CapabilitySpec(
            capability="gtm_analytics", provider="gtm", tool="scoreboard",
            permission="READ_ONLY",
            input_schema=_schema([], {}),
            output_schema=_schema([], {"metrics": "object"}),
            side_effect="none", approval_required=False,
            factory_stages=["MEASURE", "LEARN"],
            status="INTEGRATED", reason="measured events only; existing scoreboard/attribution tests",
        ),
    ]
...
def coverage_report(registry: list[CapabilitySpec]) -> dict[str, list[str]]:
    ...
    tested = {
        ...
        "market_research", "company_research", "lead_cleaning",
        "lead_qualification", "account_scoring", "offer_matching",
        "message_generation", "outreach_drafting", "response_classification",
        "crm_update", "pipeline_management", "sales_brief", "gtm_analytics",
    }
