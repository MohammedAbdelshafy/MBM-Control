...
def load_offer(offer_dir: Path) -> dict[str, Any]:
    ...
    whop_data = _load_json(whop) if whop.exists() else {}
    items = whop_data.get("items", {})
    payment_ready = isinstance(items, dict) and any(
        isinstance(item, dict) and bool(item.get("checkout_url")) for item in items.values()
    )
...
def main() -> int:
    ...
    print(json.dumps(build_customer_pack(args.offer_dir, args.output_dir), indent=2, ensure_ascii=False))
    return 0
