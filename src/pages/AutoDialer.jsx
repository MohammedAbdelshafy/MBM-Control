          ...
          <div className="mt-5 rounded-2xl border border-sky-400/20 bg-sky-500/5 p-4">
            ...
            <div className="grid grid-cols-2 gap-2 mb-3 text-[11px]">
              <div className="rounded-lg bg-black/20 px-3 py-2">
                <div className="text-slate-500">OWNER DATABASE</div>
                <div className="flex items-center gap-1 font-semibold text-emerald-300">{currentLead.database_ownership_verified ? 'Verified' : 'Unverified'} <CheckCircle className="w-3 h-3" /></div>
              </div>
              <div className="rounded-lg bg-black/20 px-3 py-2">
                <div className="text-slate-500">LIVE IDENTITY</div>
                {currentLead.caller_identity_verified ? <div className="flex items-center gap-1 font-semibold text-emerald-300">Confirmed <CheckCircle className="w-3 h-3" /></div> : <div className="flex items-center gap-1 font-semibold text-amber-300">Unconfirmed</div>}
              </div>
            </div>

            <div className="text-[10px] uppercase tracking-wider text-slate-400 mb-1.5">WHO ANSWERED?</div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mb-3">
              {[['OWNER', 'Owner'], ['AUTHORIZED_DECISION_MAKER', 'Authorized Decision Maker'], ['RELATIVE_OR_ASSOCIATE', 'Relative / Associate'], ['TENANT', 'Tenant'], ['WRONG_PERSON', 'Wrong Person'], ['WRONG_NUMBER', 'Wrong Number'], ['UNKNOWN', 'Unknown']].map(([val, label]) => (
                <button key={val} type="button" onClick={() => {
                  setIdentityRelationship(val);
                  setIdentityWrongNumber(val === 'WRONG_NUMBER');
                  setIdentityDnc(false);
                  if (val === 'WRONG_PERSON' || val === 'WRONG_NUMBER' || val === 'TENANT') setIdentityNameConfirmed(false);
                  if (val === 'OWNER') setIdentityNameConfirmed(true);
                }} className={`text-[11px] px-2 py-2 rounded-lg border text-left ${identityRelationship === val ? 'border-sky-400/40 bg-sky-500/15 text-sky-200' : 'border-white/10 bg-white/5 text-slate-300'}`}>{label}</button>
              ))}
            </div>

            <div className="grid grid-cols-2 gap-3 mb-3">
              <div>
                <div className="text-[10px] uppercase tracking-wider text-slate-400 mb-1.5">NAME CONFIRMED</div>
                <div className="flex gap-2">
                  <button type="button" onClick={() => setIdentityNameConfirmed(true)} className={`flex-1 px-3 py-2 rounded-lg border text-xs font-semibold ${identityNameConfirmed ? 'border-emerald-400/40 bg-emerald-500/15 text-emerald-200' : 'border-white/10 bg-white/5 text-slate-300'}`}>Yes</button>
                  <button type="button" onClick={() => setIdentityNameConfirmed(false)} className={`flex-1 px-3 py-2 rounded-lg border text-xs font-semibold ${!identityNameConfirmed ? 'border-white/20 bg-white/10 text-white' : 'border-white/10 bg-white/5 text-slate-300'}`}>No</button>
                </div>
              </div>
              <div>
                <div className="text-[10px] uppercase tracking-wider text-slate-400 mb-1.5">PROPERTY CONFIRMED</div>
                <div className="flex gap-2">
                  <button type="button" onClick={() => setIdentityPropertyConfirmed(true)} className={`flex-1 px-3 py-2 rounded-lg border text-xs font-semibold ${identityPropertyConfirmed ? 'border-emerald-400/40 bg-emerald-500/15 text-emerald-200' : 'border-white/10 bg-white/5 text-slate-300'}`}>Yes</button>
                  <button type="button" onClick={() => setIdentityPropertyConfirmed(false)} className={`flex-1 px-3 py-2 rounded-lg border text-xs font-semibold ${!identityPropertyConfirmed ? 'border-white/20 bg-white/10 text-white' : 'border-white/10 bg-white/5 text-slate-300'}`}>No</button>
                </div>
              </div>
            </div>

            <input value={identityName} onChange={(e) => setIdentityName(e.target.value)} placeholder="Name (as they identified, optional):" className="w-full bg-slate-950/80 border border-white/10 rounded-xl px-3 py-2.5 text-sm outline-none mb-2" />
            <label className="flex items-center gap-2 text-xs text-slate-400 mb-2"><input type="checkbox" checked={identityDnc} onChange={(e) => { setIdentityDnc(e.target.checked); if (e.target.checked) { setIdentityWrongNumber(false); setIdentityRelationship('UNKNOWN'); } }} className="accent-red-400" /> Do not call</label>
